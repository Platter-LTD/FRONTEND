...
function SheetContent({
  ...
  return (
    <SheetPortal>
      ...
      >
        {children}
        <SheetPrimitive.Close className="absolute top-4 right-4 rounded-md p-1 opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-[#9A813F]/30">
...
function SheetHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('flex flex-col gap-1.5 border-b border-[#E8DFD0] px-6 py-5', className)}
      {...props}
    />
  )
}
...
function SheetFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('mt-auto flex flex-col gap-2 border-t border-[#E8DFD0] px-6 py-4', className)}
      {...props}
    />
  )
}
