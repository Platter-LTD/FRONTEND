...
const SECURITY_OPTIONS = [
  ...
  { id: "post-dated-cheque", label: "Post-dated Cheque" },
  { id: "none", label: "None" },
]
  ...
  const handleNext = () => {
    if (step === 1) {
      ...
      onSubmit({
        ...
        repaymentStructure,
        loanProcessingFee: removeCommas(loanProcessingFee),
        moratorium,
        downPaymentRequired,
        npa,
        charges,
    ...
    >
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          <Button
            onClick={handleNext}
            disabled={!purpose || !currency || !repaymentCycle || !loanTenure || !amortization || securityRequirements.length === 0}
      ...
      {step === 2 && (
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Loan Processing Fee (%)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="0.00"
                value={loanProcessingFee}
                onChange={(e) => handlePercentageChange(e.target.value, setLoanProcessingFee, 'loanProcessingFee')}
                className={`w-full pl-4 pr-8 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#9A813F] focus:border-transparent ${errors.loanProcessingFee ? 'border-red-500' : 'border-gray-300'}`}
              />
              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">%</span>
            </div>
            {errors.loanProcessingFee && <p className="text-red-500 text-xs mt-1">{errors.loanProcessingFee}</p>}
          </div>
