...
/** POST multipart field `file` → Create App MS logo upload. */
export async function POST(
  ...
  try {
    ...
    const response = await fetch(
      `${API_UPSTREAM_BASE}/api/v1/apps/${appId}/pwa-templates/${templateId}/assets/logo`,
      {
        method: "POST",
    ...
    if (!response.ok) {
      return NextResponse.json(
        { success: false, error: (data as { error?: string }).error || "Logo upload failed" },
        { status: response.status },
      )
    }
    return NextResponse.json(data)
  } catch (error) {
    console.error("[pwa logo upload]", error)
