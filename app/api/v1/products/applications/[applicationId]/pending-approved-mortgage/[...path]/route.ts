...
function targetUrl(applicationId: string, path: string[], search: string) {
  ...
  return `${productApiBaseUrl()}/api/v1/products/applications/${encodeURIComponent(applicationId)}/pending-approved-mortgage/${tail}${search}`
}
...
export async function GET(
  ...
  try {
    ...
    return Response.json(
      { success: false, error: (error as Error)?.message || "Failed to load mortgage fulfillment" },
      { status: 500 },
    )
  }
}
...
export async function POST(
  ...
  try {
    ...
    return Response.json(
      { success: false, error: (error as Error)?.message || "Failed to update mortgage fulfillment" },
      { status: 500 },
    )
  }
}
