...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await plataUpstreamAxios.get(`/api/v1/applications/pending${search ? `?${search}` : ""}`, {
    ...
    return NextResponse.json(
      { success: false, error: (error as Error)?.message || "Failed to load pending applications" },
      { status: 500 },
    )
  }
}
